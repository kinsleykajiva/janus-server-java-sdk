...
public class JanusVideoRoomPlugInAPI {
   ...
   *     "janus.plugin.videoroom", "data": { "videoroom": "destroyed" } } } &lt;/code&gt; &lt;/p&gt;
   *     If the room deletion is successful, "janus" will be "success" and "videoroom" will be
   *     "destroyed". If an error occurs, "janus" will be "error" and additional details may be
   *     available in the "error" field. If the specified room does not exist, null will be
   *     returned.
  ...
  /**
   * Checks if a video room exists in the Janus WebRTC server and returns a boolean indicating its
   * existence.
   *
   ...
   * @param publishers The maximum number of publishers allowed in the room. If the specified value
   ...
   *     or "error", "transaction": "&lt;transaction_id&gt;", "session_id": &lt;session_id&gt;,
   *     "handle_id": &lt;handle_id&gt;, "sender": &lt;sender_id&gt;, "plugindata": { "plugin":
   *     "janus.plugin.videoroom", "data": { "videoroom": "created" } } } &lt;/code&gt; &lt;/p&gt;
   *     If the room creation is successful, "janus" will be "success" and "videoroom" will be
   *     "created". If an error occurs, "janus" will be "error" and additional details may be
   *     available in the "error" field. If the specified room already exists, null will be
   *     returned.
   * @see #checkIfVideoRoomExistsBoolCheck(String)
   */
  public JSONObject createJanusRoom(
   ...
   *     "plugindata": { "plugin": "janus.plugin.videoroom", "data": { "videoroom": "exists" or
   *     "error" } } } &lt;/code&gt; &lt;/p&gt; If the room exists, "janus" will be "success" and
   *     "videoroom" will be "exists". If the room does not exist, "janus" will be "success" and
   *     "videoroom" will be "error". If an error occurs during the check, "janus" will be "error"
   *     and additional details may be available in the "error" field.
   * @see #checkIfVideoRoomExistsBoolCheck(String)
   */
  public JSONObject checkIfVideoRoomExists(String roomId) {
    ...
    // We have a problem here we don't know if Video room is configured to use string room id or
    // integer value only .
    // because of this one of the types will cause an exception if the type use is not accepted by
    // the plugin.
   ...
   *     "janus": "success" or "error", "transaction": "&lt;transaction_id&gt;", "session_id":
   *     &lt;session_id&gt;, "handle_id": &lt;handle_id&gt;, "sender": &lt;sender_id&gt;,
   *     "plugindata": { "plugin": "janus.plugin.videoroom", "data": { "videoroom": "list" or
   *     "error", "rooms": [ { "room": &lt;room_id&gt;, "description": "&lt;room_description&gt;",
   *     "audiocodec": "&lt;codec eg opus, pcma, pcmu, isac, g722, g726, g711, speex, silk, isac16k,
   *     isac32k, isac48k, isac128k, isac256k, ilbc, siren7, siren14, silk12, silk16, silk24,
   *     silk48&gt;", "videocodec": "&lt;codec eg vp8, h264&gt;", "fir_freq": "&lt;fir_freq&gt;",
   *     "num_participants": "&lt;fir_freq&gt;", "num_participants": &lt;number_of_participants&gt;,
   *     "is_private": true or false, "audiolevel_event": true or false, "require_e2ee": true or
   *     false, "playoutdelay_ext": true or false, "videoorient_ext": true or false,
   *     "notify_joining": true or false, "require_pvtid": true or false, "bitrate":
   *     &lt;bitrate&gt;, "bitrate_audio": &lt;audio_bitrate&gt;, "bitrate_video":
   *     &lt;video_bitrate&gt;, "created": &lt;timestamp&gt;, "permanent": true or false, "record":
   *     true or false, "rec_dir": "&lt;recording_directory&gt;" ... }, ... ] } } } &lt;/code&gt;
   *     &lt;/p&gt; If the list retrieval is successful, "janus" will be "success" and "videoroom"
   *     will be "list". If an error occurs during the retrieval, "janus" will be "error" and
   *     additional details may be available in the "error" field.
   */
   ...
   *     { "janus": "success" or "error", "transaction": "&lt;transaction_id&gt;", "session_id":
   *     &lt;session_id&gt;, "handle_id": &lt;handle_id&gt;, "sender": &lt;sender_id&gt;,
   *     "plugindata": { "plugin": "janus.plugin.videoroom", "data": { "videoroom":
   *     "listparticipants" or "error", "participants": [ { "id": &lt;participant_id&gt;, "display":
   *     "&lt;display_name&gt;", "audio_codec": "&lt;audio_codec&gt;", "video_codec":
   *     "&lt;video_codec&gt;", "feed": &lt;feed_id&gt;, ... }, ... ] } } } &lt;/code&gt; &lt;/p&gt;
   *     If the participant list retrieval is successful, "janus" will be "success" and "videoroom"
   *     will be "listparticipants". If an error occurs during the retrieval, "janus" will be
   *     "error" and additional details may be available in the "error" field.
   */
