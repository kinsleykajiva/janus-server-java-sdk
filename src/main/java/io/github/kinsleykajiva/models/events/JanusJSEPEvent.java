...
/**
 * Represents a Janus JSEP event, containing information about the emitter, event type, timestamp,
 * session ID, handle ID, opaque ID, and the nested event details.
 */
public class JanusJSEPEvent {

  /**
   * The root record for a Janus JSEP event, containing information about the emitter, event type,
  ...
  /**
   * The record representing a JSEP event within Janus, including the owner and the nested JSEP
   * details.
   */
  public record Event(String owner, @Nullable JanusJSEPEvent.Jsep jsep) {}

  /**
   * The record representing the JSEP details associated with a Janus JSEP event, including the type
   * (e.g., offer, answer) and the SDP (Session Description Protocol) content.
   *
      ...
      if (root.event().jsep().type() != null) {
        ...
        var sql =
            String.format(
                ...
                root.session_id(),
                root.handle_id(),
                root.event().jsep().type().equals("offer"),
                root.event().jsep().type().equals("answer"),
                root.event().jsep().sdp(),
                timestamp);
        sqlList.add(sql);
