  ...
  public Map<DatabaseConnection, List<String>> trackInsert() {
    ...
    if (event instanceof VideoRoomPluginEventData) {
      sql.append("INSERT INTO janus_plugins (session, handle, plugin, event, timestamp) VALUES (")
          .append(this.getSession_id())
          .append(", ")
          .append(this.getHandle_id())
          .append(", '")
          .append(this.getEvent().getPlugin())
          .append("', '")
          .append(((VideoRoomPluginEventData) event).getEvent())
          .append("', '")
          .append(timestamp)
          .append("');");
      ...
      if (((VideoRoomPluginEventData) event).getStream() != null) {
        for (int i = 0; i < ((VideoRoomPluginEventData) event).getStream().length; i++) {
          ...
          stream.put(
              "feed_display", ((VideoRoomPluginEventData) event).getStream()[i].getFeed_display());
      ...
      sql.append(
              "INSERT INTO janus_videoroom_plugin_event(session,handle,data_id,data_private_id,display,room,opaque_id,streams_array,timestamp) VALUES(")
          .append(this.getSession_id())
          .append(", ")
          .append(this.getHandle_id())
          .append(", ")
          .append(((VideoRoomPluginEventData) event).getId())
          .append(", ")
          .append(((VideoRoomPluginEventData) event).getPrivate_id())
          .append(", '")
          .append(((VideoRoomPluginEventData) event).getDisplay())
          .append("', '")
          .append(((VideoRoomPluginEventData) event).getRoom())
          .append("', '")
          .append(((VideoRoomPluginEventData) event).getOpaque_id())
          .append("', '")
          .append(streamsJ.toString(2))
          .append("', '")
          .append(timestamp)
          .append("' );");
