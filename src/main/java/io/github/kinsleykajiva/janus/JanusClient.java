...
public class JanusClient implements WebSocket.Listener {
	...
	private ServerInfo convertToServerInfo(JSONObject response) {
		try {
			...
			Map<String, ServerInfo.Dependency> dependencies = dependenciesJson.keySet().stream()
			...
			Map<String, ServerInfo.Transport> transports = transportsJson.keySet().stream()
					                                               .collect(Collectors.toMap(
							                                               key -> key,
							                                               key -> {
			...
			Map<String, ServerInfo.Plugin> plugins = pluginsJson.keySet().stream()
					                                         .collect(Collectors.toMap(
							                                         key -> key,
							                                         key -> {
